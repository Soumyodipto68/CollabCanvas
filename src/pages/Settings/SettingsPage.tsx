...
const displayNameKey = "collab-canvas-display-name";
...
export const SettingsPage: React.FC = () => {
  ...
  useEffect(() => {
    const storedDisplayName = window.localStorage.getItem(displayNameKey);
  ...
  const savePreferences = () => {
    window.localStorage.setItem(displayNameKey, displayName.trim());
          ...
          <div className="mx-auto max-w-5xl">
            ...
            <section className="mb-8 flex items-start gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
              ...
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-slate-400">Preferences are saved on this device. Your account identity is managed by the sign-in provider.</p>
              </div>
            </section>

            <div className="flex items-center justify-end gap-3 pb-8">
              {saved && <span className="flex items-center gap-1.5 text-sm text-emerald-400"><Check className="h-4 w-4" /> Saved</span>}
              <button type="button" onClick={savePreferences} className="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-blue-600/20 transition hover:bg-blue-500">Save changes</button>
            </div>
          </div>
        </main>
      </div>
